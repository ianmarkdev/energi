...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $_SESSION['checkout']['shipping_method'] = $_POST['shipping'] ?? 'Standardlieferung';
    $_SESSION['checkout']['shipping_price'] = $_POST['shipping'] === 'Expresslieferung' ? 45.99 : 0;
    header("Location: checkout-schritt-2");
    exit;
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <style>
    ...
    .option-box { border:1px solid #e2e8f0; border-radius:10px; padding:1rem; margin-bottom:1rem; display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; cursor:pointer; transition:border-color 0.2s, background 0.2s; }
    ...
    .option-box small { width:100%; margin-top:0.5rem; }
    @media (max-width: 576px) {
      .option-box { gap:0.5rem; }
      .option-box > div:first-child { flex:1; min-width:0; }
      .option-box > span { flex-shrink:0; font-weight:600; }
    }
      ...
      <div class="col-lg-7">
        <form id="step1form" method="post" action="">
          <div class="checkout-card">
            ...
            <div class="option-box active" data-type="shipping" data-name="Standardlieferung" data-price="0">
              <div class="d-flex align-items-center">
                <input type="radio" name="shipping" id="shipping1" value="Standardlieferung" checked>
                <label for="shipping1" class="ms-2">Standardlieferung</label>
              </div>
              <span>Kostenlos</span>
              <small class="text-muted d-block mt-1">Lieferung innerhalb von 14 Werktagen</small>
            </div>
            <div class="option-box" data-type="shipping" data-name="Expresslieferung" data-price="45.99">
              <div class="d-flex align-items-center">
                <input type="radio" name="shipping" id="shipping2" value="Expresslieferung">
                <label for="shipping2" class="ms-2">Expresslieferung</label>
              </div>
              <span>+ 45,99 €</span>
              <small class="text-muted d-block mt-1">Bevorzugte Lieferung innerhalb von 7 Werktagen</small>
            </div>
          </div>
          ...
          <div class="sticky-footer">
            <div class="container d-flex justify-content-between align-items-center">
              <button type="submit" id="nextBtn" class="btn btn-success px-4 py-2" disabled>
                <?= $detectMobile->isMobile() ? "Weiter zur Bestätigung" : "Weiter zu: Liefertermin"; ?> &rarr;
              </button>
            </div>
          </div>
        </form>
...
$(function(){
  ...
  // Saved session data for pre-population
  const savedLieferanschriften = <?= json_encode($_SESSION['checkout']['lieferanschriften'] ?? [], JSON_UNESCAPED_UNICODE) ?>;
  const savedPayment = <?= json_encode($_SESSION['checkout']['payment_method'] ?? '') ?>;
  const savedShipping = <?= json_encode($_SESSION['checkout']['shipping_method'] ?? 'Standardlieferung') ?>;
  ...
  function calcSidebar() {
    ...
    let shippingEl = $('[data-type="shipping"].active');
    let shipping = shippingEl.data('name');
    let shippingPrice = parseFloat(shippingEl.data('price')) || 0;
    let shippingDisplay = shippingPrice > 0 ? shippingPrice.toFixed(2).replace('.',',') + ' €' : 'Kostenlos';
    $('#sidebar-delivery').html(`<div class="sidebar-row"><strong>Versand:</strong> ${shipping}</div><div class="sidebar-row"><span>Versandkosten:</span><span>${shippingDisplay}</span></div>`);
  ...
  // Pre-populate from session if returning to edit
  if (Object.keys(savedLieferanschriften).length > 0) {
    // Convert saved data to the format expected by setLieferanschriftValues
    let sessionVals = {};
    for (const [idx, adr] of Object.entries(savedLieferanschriften)) {
      sessionVals[idx] = {
        [`anrede_${idx}`]: adr.anrede || '',
        [`vorname_${idx}`]: adr.vorname || '',
        [`nachname_${idx}`]: adr.nachname || '',
        [`strasse_${idx}`]: adr.strasse || '',
        [`plz_${idx}`]: adr.plz || '',
        [`ort_${idx}`]: adr.ort || '',
        [`land_${idx}`]: adr.land || 'Deutschland',
        [`email_1`]: adr.email || '',
        [`telefon_1`]: adr.telefon || ''
      };
    }
    setLieferanschriftValues(sessionVals);
  }

  // Pre-select shipping option
  if (savedShipping) {
    $('[data-type="shipping"]').removeClass('active').find('input').prop('checked', false);
    $(`[data-type="shipping"][data-name="${savedShipping}"]`).addClass('active').find('input').prop('checked', true);
  }

  // Pre-select payment option
  if (savedPayment) {
    $('[data-type="payment"]').removeClass('active').find('input').prop('checked', false);
    $(`[data-type="payment"] input[value="${savedPayment}"]`).prop('checked', true).closest('[data-type="payment"]').addClass('active');
  }
